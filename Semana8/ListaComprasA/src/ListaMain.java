...
public class ListaMain {
    public static void main(String[] args) {
        ...
        String archivoEntrada = "ListaComprasA\\archivos\\productos.csv";
        String archivoSalida = "ListaComprasA\\archivos\\resumen_compras.txt";
        ...
        try (BufferedReader lector = new BufferedReader(new FileReader(archivoEntrada))) {
            ...
            while ((linea = lector.readLine()) != null) {
                if (!linea.trim().isEmpty()) { // si la linea no esta vacia, entra
                    ...
                    // creo un objeto de tipo Producto con los aux y lo agrego en el arraylist productos
        ...
        for (Map.Entry<Categoria, Double> entry : totalPorCategoria.entrySet()) { // entrySet me devuelve el par
            ...
            if (valor < min) {   // si valor es menor que min, valor es el nuevo min y asigno la categoria a masBarata  con getKey
                min = valor;
        ...
        try (BufferedWriter escritor = new BufferedWriter(new FileWriter(archivoSalida))){
            ...
            for(Producto p : productos){
                ...
                for (Categoria c : Categoria.values()){
                    ...
                    escritor.write(c + ": " + cantTotal + " productos - $" + String.format("%.2f", precioTotal));
                    escritor.newLine();
                }
                escritor.newLine();
...
// revisar porq me salen 9 veces el mismo texto a partir de total por categoria
// arreglar los for anidados, probar con separarlos
